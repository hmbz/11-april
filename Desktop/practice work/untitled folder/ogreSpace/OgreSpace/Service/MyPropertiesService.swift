...
class MyPropertiesService{
  static let instance = MyPropertiesService()
  lazy var status = 0
  var header:HTTPHeaders = [:]
  var totalPages:Int?
  lazy var myPropertyArray = [propertyModel]()
  //TODO:- For Recently Viewed Property Api
  func MyPropertiesServiceAPI(url:String,completion:@escaping (Int, Bool) -> ()){
        ...
        do{
          if let jsonDic = try JSON(data: data).dictionary{
            ...
            for item in jsonArray {
              ...
              self.myPropertyArray.append(object)
            }
            print(self.myPropertyArray)
